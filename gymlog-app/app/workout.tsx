...
import { saveWorkout } from '@/database/workouts';
    ...
    <View style={styles.card}>
      ...
      {item.sets.map((set, index) => (
        <View key={index} style={styles.setRow}>
          ...
          />
          {/* RIR = Reps In Reserve, ex: 2 = tu aurais pu faire 2 reps de plus */}
          <TextInput
            style={styles.input}
            placeholder="2"
            keyboardType="numeric"
            value={set.rir}
            onChangeText={(val) => updateSet(item.exercise.id, index, 'rir', val)}
          />
        </View>
      ))}
      ...
      [
        { text: 'Annuler', style: 'cancel' },
        {
          ...
          onPress: () => {
            ...
            saveWorkout(workoutExercises, duration);
            clearWorkout();
            router.replace('/(tabs)');
          },
        },
...
const styles = StyleSheet.create({
  ...
  setHeader: { flexDirection: 'row', justifyContent: 'space-between', marginBottom: 6 },
  setHeaderText: { flex: 1, textAlign: 'center', fontSize: 12, color: '#aaa', fontWeight: '600' },
  setRow: { flexDirection: 'row', alignItems: 'center', marginBottom: 8 },
  setNumber: { flex: 1, textAlign: 'center', fontSize: 15, color: '#555' },
  input: {
    flex: 1, borderWidth: 1, borderColor: '#e0e0e0', borderRadius: 8,
    padding: 8, textAlign: 'center', marginHorizontal: 4, fontSize: 15,
  },
  addSetButton: { marginTop: 8, alignItems: 'center', padding: 8 },
  addSetText: { color: '#E63946', fontWeight: '600' },
  empty: { flex: 1, alignItems: 'center', justifyContent: 'center' },
  emptyText: { fontSize: 16, color: '#888' },
  emptyLink: { marginTop: 12, color: '#E63946', fontSize: 16, fontWeight: '600' },
  footer: { padding: 16, gap: 12 },
  addExerciseButton: {
    borderWidth: 1, borderColor: '#E63946', borderRadius: 12, padding: 14, alignItems: 'center',
  },
  addExerciseText: { color: '#E63946', fontWeight: '600', fontSize: 16 },
  finishButton: { backgroundColor: '#E63946', borderRadius: 12, padding: 14, alignItems: 'center' },
